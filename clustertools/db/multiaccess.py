...
__author__ = "G. H. Collin, C. Lassner"
...
class Streamer:
    ...
    def __read_process(self, path, read_size, cbuf, stop, barrier, cyclic, offset, read_skip, sync):
        ...
        ary = h5_file[path]
        nsamples, colnames, coltypes = cdb.parse_dset(ary)
        colnames_filtered = []
        coltypes_filtered = []
        for cn, ct in zip(colnames, coltypes):
            if ct != 'text':
                colnames_filtered.append(cn)
                coltypes_filtered.append(ct)
        cdt = None

        i = offset
        while not stop.is_set():
            decoded = []
            for dec_idx in range(read_size):
                decoded.append(tuple(cdb.decode(ary, (i+dec_idx) % (len(ary)),
                                                colnames_filtered, coltypes_filtered,
                                                as_list=True)))
            if cdt is None:
                # Build compound datatype.
                dtlist = []
                for cn, ct, dec in zip(colnames_filtered, coltypes_filtered, decoded[0]):
                    if ct == 'manual':
                        dtlist.append((str(cn), h5_node[cn].dtype.type, h5_node[cn].dtype.shape))
                    else:
                        dtlist.append((str(cn),
                                       np.uint8,
                                       dec.shape))
                cdt = np.dtype(dtlist)
            vals = np.array(decoded, dtype=cdt)
    ...
    def __get_batch(self, path, length, last=False):
        ...
        import clustertools.db.tools as cdb
        h5_file = h5py.File(self.filename, 'r')
        h5_node = h5_file[path]
        nsamples, colnames, coltypes = cdb.parse_dset(h5_node)
        # For high speed access, we need same size arrays, that's why variable
        # length text is filtered.
        colnames_filtered = []
        coltypes_filtered = []
        dtlist = []
        for cn, ct in zip(colnames, coltypes):
            if ct != 'text':
                ...
                LOGGER.warn('Ignoring column `%s`!', cn)
        ...
        if length is None:
            chunkshape = h5_node[colnames[0]].chunks
        ...
        if last:
            start = length*(len(h5_node)//length)
            end = len(h5_node)
        else:
            start = 0
            end = length
        decoded = []
        for dec_idx in range(start, end):
            decoded.append(tuple(cdb.decode(h5_node, dec_idx,
                                            colnames_filtered, coltypes_filtered,
                                            as_list=True)))
        # Build compound datatype.
        for cn, ct, dec in zip(colnames_filtered, coltypes_filtered, decoded[0]):
            if ct == 'manual':
                ...
                dtlist.append((str(cn),
                               np.uint8,
                               dec.shape))
        decoded = np.array(decoded, dtype=np.dtype(dtlist))
        h5_file.close()
        return decoded
